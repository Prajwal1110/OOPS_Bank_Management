...
public:
    int accountNumber;
    double balance;
    // The Account class declares pointers to Insurance and Loan objects because an account can be associated with insurance policies and loans.
    Insurance *insurance;
    Loan *loan[3];
    int i = 0;
...
public:
    ...
    void removeAccount(int accNo)
    {
        ...
        // Loop through the accounts to find the one with the given account number (accNo)
        for (i = 0; i < accountCount; i++)
        {
            if (account[i]->accountNumber == accNo)
            { // If the account is found
                ...
                // Shift all elements after the found account to the left
                // This effectively removes the found account by overwriting it with the next one
                for (int j = i; j < accountCount - 1; j++)
                {
                    account[j] = account[j + 1];
                }

                accountCount--; // Decrement the account count since one account is removed

                // Set the last element to NULL (after shifting) to avoid dangling pointers
                // The last position is now empty, so we set it to NULL to indicate no account is there
                account[accountCount] = NULL;

                return; // Exit the function after removing the account
            }
        }
